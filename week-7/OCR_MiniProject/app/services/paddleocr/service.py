"""PaddleOCR service (Phase 6).

Runs the engine notebook in ``notebooks/paddleocr_engine.ipynb`` and
...
class OCRAvailabilityError(RuntimeError):
    """Raised when the OCR engine cannot be loaded and no stub is acceptable."""


class PaddleOCRService:
    """High-level wrapper around the PaddleOCR notebook engine.

    The service lazily loads the engine on first use, runs Phase 5
    preprocessing by default, and converts PaddleOCR's nested-list
    output into the typed :class:`OCRResult` shape used by the rest
    of the portal.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._notebook_path = Path(
            notebook_path or settings.paddleocr_notebook_abs
        ).expanduser()
    ...
    def recognize(
        ...
        *,
        ...
        """Run PaddleOCR over ``image`` and return a typed result.

        ``image`` may be a file path, raw bytes, a BGR ndarray, or a
        :class:`PreprocessResult` from Phase 5.
        """
        # 1. Materialize the BGR image (and optionally preprocess it).
        bgr, pre_meta = self._materialize_image(image)

        # 2. Load the engine. Falls back to a stub if the notebook can't run.
        ns = self._try_load_engine()
        stub_reason: str | None = None
        if ns is None:
            stub_reason = (
                f"paddleocr engine notebook failed to execute: "
                f"{self._last_load_error}"
            )
            logger.warning("paddleocr unavailable: %s", stub_reason)
            return self._empty_result(language, stub_reason=stub_reason)

        # 3. Run OCR. The engine's `recognize` returns a dict with
        #    `boxes` (list of [polygon, text, conf]) and `elapsed_ms`.
        lang = language or settings.paddleocr_lang
        try:
            output = ns["recognize"](bgr, engine=ns["engine"], lang=lang)
        except Exception as exc:  # pragma: no cover - notebook-defined behavior
            logger.exception("paddleocr.recognize failed: %s", exc)
        ...
        # Spatial sorting: cluster by Y (lines), then sort by X (columns)
        def get_center(box: OCRBox) -> tuple[float, float]:
            xs = [pt[0] for pt in box.polygon]
        ...
        if boxes:
            ...
            sorted_boxes = []
            for line in lines:
                line.sort(key=lambda b: get_center(b)[0])
        ...
        if pre_meta:
            ...
            logger.info(
                "preprocess→ocr: %dx%d → %d boxes (threshold=%.2f)",
                width_in,
                height_in,
                len(boxes),
                threshold,
    ...
    def _materialize_image(
        ...
        """Return a (BGR ndarray, preprocessing_meta) tuple.

        If ``image`` is a :class:`PreprocessResult` from Phase 5, the
        already-cleaned image is used and the metadata is forwarded.
        """
        # PreprocessResult duck-typing (avoid an import cycle).
    ...
    def _try_load_engine(self) -> dict[str, Any] | None:
        """Run the engine notebook and return its namespace, or None on failure."""
    ...
    def _empty_result(
        ...
        return OCRResult(
            language=language or settings.paddleocr_lang,
            boxes=[],
            full_text="",
            elapsed_ms=0.0,
...
__all__ = ["OCRBox", "OCRAvailabilityError", "OCRResult", "PaddleOCRService"]
